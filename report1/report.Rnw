...
\title{Neural Networks Preliminary Assignment 1}
...
\end{cases} \]
as $N \to \infty$, meaning that there is a point at $\alpha = 2$ where the data is not linearly seperable any more. The perceptron will not be able to separate the datapoints anymore. The findings of the experiment are shown in \autoref{fig:plot}, here it can be seen that at around $\alpha = 2$ that the network is able to separate about half of the generated datasets.

The storage performance mostly degenerates before the $\alpha = 2$ line, with $Q_\text{l.s.} \approx 0.25$ at the point where the transition is expected to happen.
